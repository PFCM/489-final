...
2011 as a simpler way of representing a slightly earlier tree based form derived from a
...
of a three-way tensor of size \(I \times J \times K\) has three `cores' with shapes
\(1 \times I \times r_1\), \(r_1 \times J \times r_2\) and \(r_2 \times K \times 1\). For
...
shapes are less consistent: \(\mat{A}\) is an \(I \times r_1\) matrix, \(\tensor{B}\) a
\(r_1 \times J \times r_1\) tensor and \(\mat{C}\) an \(r_2 \times K\) matrix.
...
tensor product, albeit involving a smaller tensor. We denote the product as
...
product to a matrix product. To the best of our knowledge this result has not been presented 
elsewhere. It can be found in the appendix, proposition~\ref{prop:equiv}.
